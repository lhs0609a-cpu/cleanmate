...
export interface FileEntry {
  ...
  ageDays: number
  /**
   * 이 파일의 '실물 신원'. 여러 경로가 같은 값이면 **이미 같은 실물**이다(하드링크).
   *
   * ★ 왜 필요한가 (2026-08-18 실측): sd_xl_base_1.0.safetensors 6.46GB가
   *   6개 경로에 있었다. 폴더 크기를 더하면 38.76GB로 나오지만 디스크가 실제로
   *   쓰는 건 6.46GB고, 하나를 지워도 **1바이트도 안 빈다.**
   *   내용이 같은 것과 자리를 따로 차지하는 것은 다른 문제다.
   *
   * 링크가 하나뿐이면(대부분) 비워둔다 — 42만 개에 문자열을 다 만들 이유가 없다.
   * 스캐너가 이미 stat을 부르므로 얻는 비용은 0이다.
   */
  ino?: string
}
