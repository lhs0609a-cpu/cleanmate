...
test('★ 창이 화면에 맞춰 열린다 — 잘리지도, 화면 밖으로 넘치지도 않게', () => {
  /* ★ 전에는 maximized: true를 요구했다. 목록이 잘리는 걸 막으려던 것인데,
     실물에서 반대쪽으로 터졌다 — "창이 너무 크게 열린다". 배율 150% 화면
     (1920×1080 → 논리 1280×720)에서 최대화는 곧 전체화면이고, 정리 앱 하나가
     화면을 통째로 먹는다. 높이 820도 그 화면에는 아예 안 들어간다.

     그래서 '크게'가 아니라 '맞게' 연다. 양쪽 끝을 다 박아둔다 —
     한쪽만 박으면 다음에 또 반대쪽으로 넘어간다. */
  const conf = JSON.parse(read('src-tauri/tauri.conf.json'))
  const w = conf.app.windows[0]

  assert.ok(!w.maximized, '최대화로 열면 화면을 통째로 먹는다 — 정리 앱은 주인공이 아니다')

  /* 아래쪽: 왼쪽 메뉴(248px) + 본문(.wrap 960px)이 가로 스크롤 없이 들어가야 한다.
     여기보다 좁으면 3열짜리 표가 뭉갠다(app.html .cal-wrap·.split3). */
  assert.ok(w.width >= 1100, `기본 너비가 좁다(${w.width}) — 메뉴 248 + 본문 960이 안 들어간다`)

  /* 위쪽: 가장 흔한 작은 '논리' 해상도가 1280×720이다(1920×1080 @150%).
     거기에 제목 표시줄과 작업 표시줄까지 두고 들어가야 '적당히' 열린 것이다. */
  assert.ok(w.width <= 1280, `기본 너비가 넓다(${w.width}) — 1280 논리 화면에서 넘친다`)
  assert.ok(w.height <= 680, `기본 높이가 크다(${w.height}) — 720 논리 화면에서 아래가 잘린다`)

  // 좁혀 쓰는 사람도 있다. 다만 여기보다 좁아지면 메뉴와 본문이 겹친다.
  assert.ok(w.resizable, '크기를 못 바꾸면 위 숫자가 전부 강요가 된다')
  assert.ok(w.minWidth >= 800, `최소 너비가 너무 작다(${w.minWidth})`)
})
