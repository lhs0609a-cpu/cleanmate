...
test('★ "관리자 권한이 필요해요"로 끝내지 않는다 — 여기서 끌 수 있어야 한다', () => {
  /* 예전 화면은 모든 사용자용 항목에 딱지만 붙였다. 사용자가 할 수 있는 게 없었고,
     "그럼 작업관리자로 가세요"라는 말조차 화면 밖에 있었다.
     권한이 필요하면 권한을 물어보면 된다 — 그게 restore-measure에서 배운 것이다. */
  const body = loadStartupBody(app())

  assert.doesNotMatch(
    body,
    /모든 사용자용이라 관리자 권한이 필요해요/,
    '아직 딱지만 붙이고 있다 — 알려만 주고 세워두는 건 안내가 아니다'
  )
  // 승격이 필요한 항목에도 누를 수 있는 버튼이 나가야 한다.
  assert.match(
    body,
    /e\.needsAdmin/,
    '승격이 필요한 항목을 화면이 구분하지 않는다'
  )
  assert.match(
    body,
    /확인 창/,
    '확인 창(UAC)이 뜬다는 걸 누르기 전에 말하지 않는다'
  )
})
