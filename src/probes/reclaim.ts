...
export function probeRecycleBin(f: ReclaimFacts): Finding | null {
  ...
  return {
    ...
    explain: {
      ...
      why:
        '윈도우는 파일을 지울 때 바로 없애지 않고 휴지통으로 옮깁니다. 되돌릴 수 있게 하려고요. ' +
        '그래서 "지웠는데 용량이 그대로"인 겁니다 — 아직 안 지워졌거든요.',
      usedBy: [
        '되돌리기 — 실수로 지운 파일을 원래 자리로 돌려놓는 기능이에요. 비우면 이 기능이 대상 없이 빕니다.',
      ],
      ifRemoved: [
        `${size(f.recycleBytes)}가 실제로 비워집니다. 숨은 공간 중에 즉시 효과가 가장 확실한 항목이에요.`,
        '★ 되돌릴 수 없습니다. 휴지통을 비우면 그 안의 파일은 저희 격리함으로도 못 가져옵니다.',
      ],
      recovery: 'none',
      recoveryNote:
        '아니요. 비우면 끝입니다. 그래서 저희가 알아서 비우지 않고, 목록을 먼저 열어보시라고 안내만 드려요.',
      ifKept: `아무 문제 없습니다. ${size(f.recycleBytes)}를 계속 쓸 뿐이고, 그동안은 언제든 되돌릴 수 있어요.`,
    },
    assist: {
      label: '휴지통 비우기',
...
export function probeUpdateCache(f: ReclaimFacts): Finding | null {
  ...
  return {
    ...
    explain: {
      what:
        '윈도우가 업데이트를 받아서 설치한 뒤, 그 설치 파일을 지우지 않고 남겨둔 것들이에요. ' +
        `${f.systemDrive}\\Windows\\SoftwareDistribution\\Download 에 ${size(f.updateCacheBytes)}가 쌓여 있습니다.`,
      why:
        '설치가 실패했을 때 다시 쓰려고 남겨둡니다. 그런데 성공한 뒤에도 한동안 그대로라, ' +
        '업데이트를 여러 번 받은 PC일수록 커집니다. 탐색기로는 이 폴더를 들여다보기도 어려워요.',
      usedBy: [
        '윈도우 업데이트 — 설치 중이거나 되돌리기가 필요할 때 이 파일들을 씁니다.',
        '업데이트 재시도 — 실패한 업데이트를 다시 시도할 때 여기 있는 파일을 재사용해요.',
      ],
      ifRemoved: [
        '다음 업데이트를 받을 때 일부를 다시 내려받아야 할 수 있어요(그만큼 데이터를 더 씁니다).',
        '업데이트 되돌리기가 필요한 상황이면 그 자료가 사라집니다.',
      ],
      recovery: 'auto-regenerates',
      recoveryNote:
        '필요해지면 윈도우가 다시 받습니다. 다만 저희가 직접 지우지는 않아요 — ' +
        '이 폴더는 관리자 권한이 필요하고, 업데이트 서비스가 쓰고 있는 중일 수 있습니다.',
      ifKept: `아무 문제 없습니다. ${size(f.updateCacheBytes)}를 계속 쓸 뿐이에요.`,
    },
    assist: {
      label: '윈도우 디스크 정리 열기',
      ...
      irreversible: false,
      note:
        '윈도우가 만든 정식 도구(디스크 정리)를 띄웁니다. 거기서 "Windows 업데이트 정리"를 고르시면 돼요. ' +
        '저희가 이 폴더를 직접 건드리지 않는 이유는 지우다 실패하면 업데이트가 깨지기 때문입니다.',
    },
  }
}
